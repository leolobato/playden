...
public actor GamesVolumeStore: VolumeManaging {
    ...
    public func availableVolumes() throws -> [GamesVolume] {
        ...
        for path in [home] + mounted {
            ...
            let root = isHome ? home.appendingPathComponent("Games/Playden", isDirectory: true) : mount.appendingPathComponent("Playden/games", isDirectory: true)
    ...
    @discardableResult private func checkWriteCommand(_ executable: String, _ arguments: [String]) async throws -> CommandResult {
        ...
        guard result.exitCode == 0 else { throw OperationFailure(stage: "Games volume", reason: "Playden couldn’t write to this drive. Allow access in macOS settings, or choose another drive.", output: result.output) }
