...
public actor CrossOverGameBottles: GameBottleManaging {
    ...
    public func prepare(_ bottle: GameBottle) async throws {
        ...
        try checkConfiguration(destination)
        let result = try await run("cxstart", ["--bottle", destination.path, "--no-gui", "--wait-children", "cmd.exe", "/c", "echo PLAYDEN_GAME_BOTTLE_READY"], timeout: 45)
        guard result.output.contains("PLAYDEN_GAME_BOTTLE_READY") else { throw problem("The game's runtime did not finish its startup check.", output: result.output) }
        try BottleFolders.verify(destination)
        try Task.checkCancellation()
    ...
    private func readMarker(at directory: URL, matching bottle: GameBottle) throws -> Marker {
        ...
        guard (try? file.resourceValues(forKeys: [.isSymbolicLinkKey]).isSymbolicLink) == false,
              ...
              marker.bottle == bottle else { throw problem("This runtime folder does not belong to this Playden installation. Its files have been kept.") }
        return marker
    }
