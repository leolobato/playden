...
public actor CrossOverRuntime: BottleManaging {
    ...
    public func prepareTemplate(onProgress: @escaping @Sendable (TemplateStage) -> Void = { _ in }) async throws -> RuntimeInfo {
        ...
        do {
            ...
            if let saved = try loadReceipt() { receipt = saved }
                ...
                guard !exists(template) else { throw issue("Create template", "A bottle with the setup name already exists and does not belong to Playden.") }
            ...
            let result = try await commands.run(executable: tool("cxstart"), arguments: ["--bottle", templateName,
                "--no-gui", "--wait-children", "cmd.exe", "/c", "echo PLAYDEN_TEMPLATE_READY"], timeout: 45)
            try requireSuccess(result, stage: stage)
            guard result.output.contains("PLAYDEN_TEMPLATE_READY") else { throw issue(stage, "The game runtime did not finish its startup check.", output: result.output) }
    ...
    private func checkRuntime() throws -> String {
        ...
        guard let version = properties?["CFBundleShortVersionString"] as? String,
            ...
            throw issue("Check runtime", "Playden needs CrossOver 26 or newer.")
        }
        return version
    }
    ...
    private func safeTemplate() throws {
        ...
        guard values.isSymbolicLink != true, values.isDirectory == true,
            ...
            throw issue("Check template", "The game setup folder has moved or is not owned by Playden.")
        }
    }
...
private struct BottleOwner: Codable, Equatable {
    ...
    var version: String
    var token: UUID
    var description: String { "Playden managed template \(token.uuidString)" }
}
