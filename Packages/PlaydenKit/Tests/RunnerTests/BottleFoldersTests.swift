...
final class BottleFoldersTests: XCTestCase {
    private func root() throws -> URL {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("Playden-folders-\(UUID().uuidString)")
    ...
    func testPublicationRewritesFolderTargetsToFinalBottle() throws {
        ...
        let mapping = try String(contentsOf: destination.appendingPathComponent(".playden-folders/user-dirs.dirs"), encoding: .utf8)
    ...
    func testLinkedProfileAndConfigurationAreRefused() throws {
        ...
        try FileManager.default.createSymbolicLink(at: bottle.appendingPathComponent(".playden-folders"), withDestinationURL: outside)
        XCTAssertThrowsError(try BottleFolders.configure(bottle))
        XCTAssertTrue(try FileManager.default.contentsOfDirectory(atPath: outside.path).isEmpty)
        try FileManager.default.removeItem(at: bottle.appendingPathComponent(".playden-folders"))
