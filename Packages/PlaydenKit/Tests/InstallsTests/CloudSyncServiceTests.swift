...
final class CloudSyncServiceTests: XCTestCase {
    ...
    func testPullEditPushAndReopenUseRealJournalAndFilesystem() async throws {
        ...
        XCTAssertEqual(pulled.latestCloudSaveAt, payload("remote progress").file.modifiedAt)
        ...
        XCTAssertEqual(pushed.latestCloudSaveAt, remote.files.filter { $0.state == .present }.map(\.modifiedAt).max())
