...
final class SteamAccountTests: XCTestCase {
    ...
    func testLiveQRChallengeAndPublicMetadataWhenRequested() async throws {
        guard ProcessInfo.processInfo.environment["PLAYDEN_STEAM_NETWORK_PROBE"] == "1" else {
            throw XCTSkip("Set PLAYDEN_STEAM_NETWORK_PROBE=1 for the unauthenticated network probe")
        }
    ...
    func testKeychainRoundTripUsesOnlyItsOwnService() throws {
        let first = KeychainCredentials(service: "org.lobato.playden.tests.\(UUID().uuidString)")
        let second = KeychainCredentials(service: "org.lobato.playden.tests.\(UUID().uuidString)")
    ...
    func testLegacyCredentialsMigrateOnceAndSignOutCannotRestoreThem() throws {
        let legacy = KeychainCredentials(service: "org.lobato.playden.tests.legacy.\(UUID().uuidString)")
        let current = KeychainCredentials(service: "org.lobato.playden.tests.current.\(UUID().uuidString)", legacyService: legacy.service)
