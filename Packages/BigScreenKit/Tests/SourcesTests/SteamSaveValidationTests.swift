...
final class SteamSaveValidationTests: XCTestCase {
    ...
    func testPreservedAcceptanceTitleCopiesWhenExplicitlyProvided() throws {
        ...
        guard let local = environment["BIGSCREEN_SHORT_HIKE_LOCAL_SAVE"], let remote = environment["BIGSCREEN_SHORT_HIKE_CLOUD_SAVE"] else {
            throw XCTSkip("Set the two BIGSCREEN_SHORT_HIKE_*_SAVE paths to test private preserved copies read-only")
        }
