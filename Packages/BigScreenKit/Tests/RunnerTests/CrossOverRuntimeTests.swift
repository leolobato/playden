...
private actor TemplateCommands: CommandExecuting {
    ...
    func run(executable: URL, arguments: [String], timeout: TimeInterval) async throws -> CommandResult {
        ...
        return CommandResult(exitCode: 0, output: "BIGSCREEN_TEMPLATE_READY\n")
    }
}
private struct RuntimeFixture {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent("BigScreen-runtime-test-\(UUID().uuidString)")
...
final class CrossOverRuntimeTests: XCTestCase {
    ...
    func testRealCrossOverTemplateWhenRequested() async throws {
        guard ProcessInfo.processInfo.environment["BIGSCREEN_CROSSOVER_TEMPLATE_PROBE"] == "1" else { throw XCTSkip("Opt in to creating and deleting a unique CrossOver test template") }
