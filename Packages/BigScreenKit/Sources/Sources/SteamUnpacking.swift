...
enum SteamUnpacking {
    ...
    static func executable() throws -> URL {
        guard let root = Bundle.module.url(forResource: "Steamless", withExtension: nil) else { throw failure("The game preparation tool is missing. Reinstall Big Screen.") }
        let data = try Data(contentsOf: root.appendingPathComponent("checksums.json"))
        guard hex(data) == "817b6edd5c8adaba777eb4d1a4f88ccbe01a5460bd3919260ba6327ebd6fbdef" else { throw failure("The game preparation tool could not be verified. Reinstall Big Screen.") }
        for (path, checksum) in try JSONDecoder().decode([String: String].self, from: data) {
            guard hex(try Data(contentsOf: root.appendingPathComponent(path))) == checksum else { throw failure("The game preparation tool changed. Reinstall Big Screen.") }
        }
    ...
    static func unpack(_ original: URL, in bottle: GameBottle, tools: any RuntimeToolRunning) async throws -> Data {
        ...
        let work = FileManager.default.temporaryDirectory.appendingPathComponent("BigScreen-unpack-" + UUID().uuidString)
