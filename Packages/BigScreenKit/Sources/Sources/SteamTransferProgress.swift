...
final class SteamTransferProgress: @unchecked Sendable {
    ...
    func assembled(depot: UInt32, completed: Int64, fresh: Int64?, file: String) {
        lock.lock()
        self.completed = max(self.completed, completed); self.file = file
        if let fresh { written[depot] = max(written[depot, default: 0], fresh) }
        let value = snapshot(); lock.unlock(); report(value)
    }
    private func snapshot() -> InstallProgress {
        .init(bytesCompleted: completed, bytesTotal: total, currentFile: file, downloadedBytes: downloaded,
            freshlyWrittenBytes: written.values.reduce(0, Self.add))
    }
