...
extension CatalogStore {
    ...
    private static func checkAppReset(_ db: Database) throws {
        ...
        guard sessions.allSatisfy({ $0.endedAt != nil }) else { throw resetIssue("Close the game and let its save sync finish before resetting Big Screen.") }
        let jobs: [JobRecord] = try values(db, table: "jobs")
        for job in jobs where ![.completed, .cancelled].contains(job.state) {
            guard job.kind != .uninstall else { throw resetIssue("Finish or retry the pending game removal before resetting Big Screen.") }
            guard job.cancellationRequested != true,
                  job.state == .failed || (job.state == .paused && job.pauseReasons.contains(.user)) else {
                throw resetIssue("Pause each download or file verification in Downloads before resetting Big Screen.")
            }
        }
        ...
        guard cloud.allSatisfy({ $0.phase.isTerminal || $0.claim == nil }) else {
            throw resetIssue("Let save sync finish before resetting Big Screen. Pending saves will be kept.")
        }
    }
    private static func resetIssue(_ reason: String) -> OperationFailure {
        .init(stage: "Reset Big Screen", reason: reason, output: "")
    }
}
