...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSMenuItemValidation, LauncherWindowControlling {
            ...
            model.immersiveDisplay.acquire = { [weak self] target in
                ...
                do {
                    return try await model.displayPresentation.acquire(target: target, forGame: false) { target in
                        try await TemporaryPrimaryDisplay.acquire(target: target, helper: helper, disconnectOtherDisplays: true)
                    }
                } catch let failure as OperationFailure {
                    throw failure
                } catch {
                    throw OperationFailure(stage: "Immersive mode", reason: error.localizedDescription, output: "")
                }
            }
            model.immersiveDisplay.interrupted = { [weak self] in
                self?.model.exitImmersiveAfterDisplayLoss()
            }
    ...
    private func updateImmersiveDisplay() {
        ...
        // The helper owns unplug recovery. Do not replace its target with a fallback
        // before it has re-enabled the disconnected monitors and ended the lease.
        if model.immersiveMode, let uuid = model.immersiveDisplay.requestedUUID,
           !launcherScreens.contains(where: { $0.uuid == uuid }) { return }
        ...
        model.immersiveDisplay.update(target: target)
    }
    ...
    private func captureScreens(to directory: URL) async {
        do {
            ...
            for screen in ["first-run-feedback", "first-run-feedback-issues", "settings-volumes", "game-settings", "game-more", "game-settings-more", "game-settings-custom", "picker-graphics", "profile-chooser", "toast-settings", "cloud-timestamp", "launcher-quit", "launcher-quit-warning", "exit-overlay-warning", "launcher-quitting", "game-drive-disconnected", "game-drive-checking", "context-drive-disconnected", "library-drive-disconnected", "toast-complete", "toast-failed", "toast-connected", "toast-disconnected", "uninstall-confirm", "uninstall-unsynced", "uninstall-checking", "cloud-ready", "cloud-conflict", "cloud-account", "cloud-pending", "cloud-syncing", "cloud-recovery", "home", "home-tabs", "home-library-card", "home-playstation", "home-large-library", "home-large-library-end", "home-collection-end", "library-large", "library-large-end", "library", "library-playstation", "library-paged", "library-return", "game", "game-status-clean", "game-status-crash", "library-running", "context-uninstall", "downloads", "downloads-queued", "settings", "settings-about", "settings-reset", "settings-reset-blocked", "settings-reset-error", "settings-reset-busy", "settings-display", "settings-runtime", "settings-runtime-missing", "settings-runtime-busy", "collections", "keyboard", "keyboard-playstation", "keyboard-generic", "keyboard-space", "keyboard-long", "compatibility", "uninstall", "logs", "logs-retry", "logs-long", "logs-long-end", "logs-long-return", "signin-qr", "signin-password", "signin-error", "setup-controller", "setup-permissions", "setup-display", "setup-volume", "setup-runtime", "setup-error", "setup-ready", "controller-test", "controller-waiting", "library-filters", "library-filters-bottom", "library-download-glyph", "library-download-focused", "library-artwork-fallback", "game-unknown-size", "game-favorite", "install-offer", "install-offer-space", "install-queue", "install-verifying", "install-verifying-all", "install-history-failed", "install-history-completed", "install-mini-progress", "install-storage-shortage", "install-storage-unavailable", "install-game-progress", "launching", "exit-overlay", "exit-overlay-quit", "notification", "notification-focused", "notification-signin"] {
                ...
                case "first-run-feedback", "first-run-feedback-issues":
                    model.selectTab(.home)
                    if let game = model.games.first {
                        model.show(.firstRunFeedback(game.id))
                        if screen == "first-run-feedback-issues" { model.firstRunRating = .playable; model.panelIndex = 3 }
                    }
                case "settings-volumes":
                    model.selectTab(.settings); model.settingsSection = 1; model.settingsIndex = 1
                    let root = URL(fileURLWithPath: "/Volumes/Games/Playden/games")
                    model.availableVolumes = [
                        GamesVolume(id: "ssd", name: "Games SSD", mountURL: root, gamesRoot: root, freeBytes: 812_000_000_000, totalBytes: 1_000_000_000_000),
                        GamesVolume(id: "mac", name: "This Mac", mountURL: URL(fileURLWithPath: "/"), gamesRoot: URL(fileURLWithPath: "/Users/player/Playden/games"), freeBytes: 206_000_000_000, totalBytes: 500_000_000_000)
                    ]
                    model.gamesVolume = GamesVolumeSelection(volumeID: "ssd", rootBookmark: Data(), lastKnownRoot: root, relativeRoot: "games")
                    model.installVolumes = [model.gamesVolume!]
