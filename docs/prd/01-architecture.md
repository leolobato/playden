...
| `Sources` | `GameSource` and `Installer` protocols; `SteamSource` + `SteamInstaller` (v1) | `SteamCore` (local path to `../GameNative-macos/swift`) |
...
| What | Where |
|---|---|
| Database, artwork cache, per-game config JSON, job state, logs | `~/Library/Application Support/GameNative BigScreen/` |
| Steam tokens | Keychain (v1). `SteamCore`'s file `TokenStore` is acceptable only while the Keychain adapter is unbuilt. |
| Game files | User-chosen games volume, default `/Volumes/VM/GameNative/games` if present, else `~/Games/GameNative`. Layout `<source>/<gameId>/<Name>/`, compatible with `SteamCore`'s `app_<appid>/<Name>/` for Steam. |
...
- **AR-STOR-1 (v1):** Nothing identity-derived (SteamID, tickets) is written outside the Keychain and
  the gbe_fork staging area inside a game's own directory. Matches `GameNative-macos` FR-AUTH-2.
